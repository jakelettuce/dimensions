...
- **Download confirmation** — every file download from a webportal is paused immediately (zero bytes to disk) and requires explicit user confirmation via an app-level modal showing filename, size, and source domain. Three options: "Save to Dimensions" (imports media files into `_media/` library, deduplicated), "Save to Downloads" (standard `~/Downloads`), or "Cancel". Portal cannot bypass, dismiss, or auto-confirm the modal. 60-second auto-cancel timeout. One modal at a time to prevent spam. Critical security boundary for V2 multiplayer
